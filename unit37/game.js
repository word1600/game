...
let selectedUnit = 'unit37'; // 기본값: Unit 37
...
/**
 * Unit 37 단어 JSON URL 후보 (로컬에서 서버 루트만 다를 때 대응)
 * 1) data/unit37.json — unit37 폴더만 루트로 띄움, Live Server 등 (unit11과 동일 패턴)
 * 2) ../data/unit37.json — 프로젝트 루트에서 /unit37/ 로 접속 (공용 data/)
 * 온라인: /game/unit37/data/unit37.json 또는 /game/data/unit37.json
 */
  ...
  return (async () => {
    ...
    for (const url of UNIT37_DATA_URLS) {
      try {
        ...
        console.log('📡 Unit 37: 시도', url, '→', response.status, response.ok);
        if (!response.ok) throw new Error('HTTP ' + response.status);
        const data = await response.json();
        if (!Array.isArray(data)) throw new Error('JSON 형식 오류');
        console.log('✅ Unit 37: 사용한 경로:', url);
        return data;
      } catch (e) {
        lastErr = e;
      }
...
function loadWords() {
  ...
  fetchUnit36WordJson()
    ...
    .catch(err => {
      console.error('❌ Unit 37: 데이터 로드 실패:', err);
      alert('Unit 37 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
}