...
let selectedUnit = 'unit21'; // 기본값: Unit 21
...
function getLatestUnitJsonFile() {
  // data 폴더 내 unit21.json 파일을 사용
  return fetch('data/unit21.json')
...
function loadWords() {
  // 단순 상대 경로 사용: 로컬/랜/배포 모두 동작
  const file = 'data/unit21.json';
  console.log('🔍 Unit 21: 단어 데이터 로딩 시작:', file);
  
  fetch(file)
    .then(response => {
      console.log('📡 Unit 21: 서버 응답:', response.status, response.ok);
      if (!response.ok) {
    ...
    .then(data => {
      console.log('✅ Unit 21: 데이터 로드 성공! 단어 수:', data.length);
      console.log('📝 Unit 21: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
    ...
    .catch(err => {
      console.error('❌ Unit 21: 데이터 로드 실패:', err);
      alert('Unit 21 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
}