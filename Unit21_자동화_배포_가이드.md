...
## ⚠️ 에이전트·자동 작업 필수 준수 (2026-04-12 반영)

**“Unit N 진행/생성”** 또는 사용자가 관행상 **온라인까지**를 기대하는 표현으로 요청한 경우, 아래를 **작업 완료의 일부**로 본다. 로컬에 폴더만 만든 시점에서 끝내지 않는다.

| 단계 | 내용 |
|------|------|
| 1 | `data/unitN.json`, `unitN/` 준비 및 `game.js` 등 경로 반영 |
| 2 | **`git commit` 후 `git push origin main`까지 성공** |
| 3 | (권장) `https://word1600.github.io/game/unitN/` 접속 또는 HEAD 요청으로 배포 확인. 직후 404면 Pages 지연(1~5분) 안내 |

**주의**

- **`git status`만으로 배포 여부를 판단하지 않는다.** Untracked/미스테이징은 “아직 원격에 없음”이지, 배포 완료와 동치가 아니다.
- 다른 브랜치/파일이 함께 수정되어 있으면 **`git add .` 대신** `data/unitN.json`과 `unitN/`만 스테이징한 뒤 커밋·푸시한다.

**재발 사례 기록**: 2026-04-12 Unit 34 작업에서 최초 응답이 푸시 없이 종료되어 사용자 재지적이 있었음 → `.cursor/rules/ufo-unit-deploy.mdc` 및 본 절 추가.

**상세 보고**: `2026-04-12_Unit34_생성_및_배포_완료_보고서.md`
...
- [ ] 스테이징: **`git add .`는 워킹트리가 깨끗할 때만.** 그 외에는 **`data/unitN.json` + `unitN/`만** `git add` (무관한 MD/파일 커밋 방지)
- [ ] 커밋 생성 (`git commit`)
- [ ] 원격 푸시 (`git push origin main`) — **푸시 성공 전에는 배포 완료로 보지 않음**
