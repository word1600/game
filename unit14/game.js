...
function getLatestUnitJsonFile() {
  // data 폴더 내 unit14.json 파일을 사용
  return fetch('/game/unit14/data/unit14.json')
...
function loadWords() {
  ...
  console.log('🔍 Unit 14: 단어 데이터 로딩 시작:', file);
  
  fetch(file)
    .then(response => {
      console.log('📡 Unit 14: 서버 응답:', response.status, response.ok);
      if (!response.ok) {
    ...
    .then(data => {
      console.log('✅ Unit 14: 데이터 로드 성공! 단어 수:', data.length);
      console.log('📝 Unit 14: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
    ...
    .catch(err => {
      console.error('❌ Unit 14: 데이터 로드 실패:', err);
      alert('Unit 14 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
} 