...
let selectedUnit = 'unit39'; // 기본값: Unit 39
...
/**
 * Unit 39 단어 JSON URL 후보 (로컬에서 서버 루트만 다를 때 대응)
 * 1) data/unit39.json — unit39 폴더만 루트로 띄움, Live Server 등 (unit11과 동일 패턴)
 * 2) ../data/unit39.json — 프로젝트 루트에서 /unit39/ 로 접속 (공용 data/)
 * 온라인: /game/unit39/data/unit39.json 또는 /game/data/unit39.json
 */
const UNIT39_DATA_URLS = ['data/unit39.json', '../data/unit39.json'];

function fetchUnit39WordJson() {
  return (async () => {
    let lastErr = null;
    for (const url of UNIT39_DATA_URLS) {
      try {
        const response = await fetch(url);
        console.log('📡 Unit 39: 시도', url, '→', response.status, response.ok);
        if (!response.ok) throw new Error('HTTP ' + response.status);
        const data = await response.json();
        if (!Array.isArray(data)) throw new Error('JSON 형식 오류');
        console.log('✅ Unit 39: 사용한 경로:', url);
        return data;
      } catch (e) {
        lastErr = e;
      }
...
function getLatestUnitJsonFile() {
  return fetchUnit39WordJson().then(data => {
...
function loadWords() {
  console.log('🔍 Unit 39: 단어 데이터 로딩 (순서대로 시도):', UNIT39_DATA_URLS.join(' → '));

  fetchUnit39WordJson()
    .then(data => {
      console.log('✅ Unit 39: 데이터 로드 성공! 단어 수:', data.length);
      console.log('📝 Unit 39: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
    ...
    .catch(err => {
      console.error('❌ Unit 39: 데이터 로드 실패:', err);
      alert('Unit 39 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
}