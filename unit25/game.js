...
let selectedUnit = 'unit25'; // 기본값: Unit 25
...
function getLatestUnitJsonFile() {
  ...
  const file = isLocal ? './data/unit25.json' : '/game/unit25/data/unit25.json';
  return fetch(file)
...
function loadWords() {
  ...
  const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
  const file = isLocal ? './data/unit25.json' : '/game/unit25/data/unit25.json';
  console.log('🔍 Unit 25: 단어 데이터 로딩 시작:', file);
  
  fetch(file)
    .then(response => {
      console.log('📡 Unit 25: 서버 응답:', response.status, response.ok);
      if (!response.ok) {
    ...
    .then(data => {
      console.log('✅ Unit 25: 데이터 로드 성공! 단어 수:', data.length);
      console.log('📝 Unit 25: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
    ...
    .catch(err => {
      console.error('❌ Unit 25: 데이터 로드 실패:', err);
      alert('Unit 25 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
}