...
let selectedUnit = 'unit44'; // 기본값: Unit 44
...
/**
 * Unit 44 단어 JSON URL 후보 (로컬에서 서버 루트만 다를 때 대응)
 * 1) data/unit44.json — unit44 폴더만 루트로 띄움, Live Server 등 (unit11과 동일 패턴)
 * 2) ../data/unit44.json — 프로젝트 루트에서 /unit44/ 로 접속 (공용 data/)
 * 온라인: /game/unit44/data/unit44.json 또는 /game/data/unit44.json
 */
const UNIT44_DATA_URLS = ['data/unit44.json', '../data/unit44.json'];

function fetchUnit44WordJson() {
  return (async () => {
    let lastErr = null;
    for (const url of UNIT44_DATA_URLS) {
      try {
        const response = await fetch(url);
        console.log('📡 Unit 44: 시도', url, '→', response.status, response.ok);
        if (!response.ok) throw new Error('HTTP ' + response.status);
        const data = await response.json();
        if (!Array.isArray(data)) throw new Error('JSON 형식 오류');
        console.log('✅ Unit 44: 사용한 경로:', url);
        return data;
      } catch (e) {
        lastErr = e;
      }
...
function getLatestUnitJsonFile() {
  return fetchUnit44WordJson().then(data => {
...
function loadWords() {
  console.log('🔍 Unit 44: 단어 데이터 로딩 (순서대로 시도):', UNIT44_DATA_URLS.join(' → '));

  fetchUnit44WordJson()
    .then(data => {
      console.log('✅ Unit 44: 데이터 로드 성공! 단어 수:', data.length);
      console.log('📝 Unit 44: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
    ...
    .catch(err => {
      console.error('❌ Unit 44: 데이터 로드 실패:', err);
      alert('Unit 44 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
}