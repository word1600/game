...
let selectedUnit = 'unit24'; // 기본값: Unit 24
...
function getLatestUnitJsonFile() {
  ...
  const file = isLocal ? './data/unit24.json' : '/game/unit24/data/unit24.json';
  return fetch(file)
...
function loadWords() {
  ...
  const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
  const file = isLocal ? './data/unit24.json' : '/game/unit24/data/unit24.json';
  console.log('🔍 Unit 24: 단어 데이터 로딩 시작:', file);
  
  fetch(file)
    .then(response => {
      console.log('📡 Unit 24: 서버 응답:', response.status, response.ok);
      if (!response.ok) {
    ...
    .then(data => {
      console.log('✅ Unit 24: 데이터 로드 성공! 단어 수:', data.length);
      console.log('📝 Unit 24: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
    ...
    .catch(err => {
      console.error('❌ Unit 24: 데이터 로드 실패:', err);
      alert('Unit 24 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
}