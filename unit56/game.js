...
let selectedUnit = 'unit56'; // 기본값: Unit 56
...
/**
 * Unit 56 단어 JSON URL 후보 (로컬에서 서버 루트만 다를 때 대응)
 * 1) data/unit56.json — unit56 폴더만 루트로 띄움, Live Server 등 (unit11과 동일 패턴)
 * 2) ../data/unit56.json — 프로젝트 루트에서 /unit56/ 로 접속 (공용 data/)
 * 온라인: /game/unit56/data/unit56.json 또는 /game/data/unit56.json
 */
const UNIT56_DATA_URLS = ['data/unit56.json', '../data/unit56.json'];

function fetchUnit56WordJson() {
  return (async () => {
    let lastErr = null;
    for (const url of UNIT56_DATA_URLS) {
      try {
        const response = await fetch(url);
        console.log('📡 Unit 56: 시도', url, '→', response.status, response.ok);
        if (!response.ok) throw new Error('HTTP ' + response.status);
        const data = await response.json();
        if (!Array.isArray(data)) throw new Error('JSON 형식 오류');
        console.log('✅ Unit 56: 사용한 경로:', url);
        return data;
      } catch (e) {
        lastErr = e;
      }
...
function getLatestUnitJsonFile() {
  return fetchUnit56WordJson().then(data => {
...
function loadWords() {
  console.log('🔍 Unit 56: 단어 데이터 로딩 (순서대로 시도):', UNIT56_DATA_URLS.join(' → '));

  fetchUnit56WordJson()
    .then(data => {
      console.log('✅ Unit 56: 데이터 로드 성공! 단어 수:', data.length);
      console.log('📝 Unit 56: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
    ...
    .catch(err => {
      console.error('❌ Unit 56: 데이터 로드 실패:', err);
      alert('Unit 56 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
}