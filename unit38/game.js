...
let selectedUnit = 'unit38'; // 기본값: Unit 38
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Unit 1과 동일: BGM 끝나면 처음부터 다시 재생 (loop 보강)
  if (bgm) {
  ...
  function initializeGame() {
    ...
    // Unit 1과 동일: 게임 시작 시 TTS 워밍업 없음 (BGM과 충돌 가능성 제거)
    isPaused = false;
    ufoSinceLastAnswer = 0;
    answerUfoSpawnCount = 0;
    problemWordCount = 0;
...
/**
 * Unit 38 단어 JSON URL 후보 (로컬에서 서버 루트만 다를 때 대응)
 * 1) data/unit38.json — unit38 폴더만 루트로 띄움, Live Server 등 (unit11과 동일 패턴)
 * 2) ../data/unit38.json — 프로젝트 루트에서 /unit38/ 로 접속 (공용 data/)
 * 온라인: /game/unit38/data/unit38.json 또는 /game/data/unit38.json
 */
const UNIT38_DATA_URLS = ['data/unit38.json', '../data/unit38.json'];

function fetchUnit38WordJson() {
  return (async () => {
    let lastErr = null;
    for (const url of UNIT38_DATA_URLS) {
      try {
        const response = await fetch(url);
        console.log('📡 Unit 38: 시도', url, '→', response.status, response.ok);
        if (!response.ok) throw new Error('HTTP ' + response.status);
        const data = await response.json();
        if (!Array.isArray(data)) throw new Error('JSON 형식 오류');
        console.log('✅ Unit 38: 사용한 경로:', url);
        return data;
      } catch (e) {
        lastErr = e;
      }
    }
    throw lastErr || new Error('단어 데이터를 불러오지 못했습니다.');
  })();
}

function getLatestUnitJsonFile() {
  return fetchUnit38WordJson().then(data => {
...
function loadWords() {
  console.log('🔍 Unit 38: 단어 데이터 로딩 (순서대로 시도):', UNIT38_DATA_URLS.join(' → '));

  fetchUnit38WordJson()
    .then(data => {
      console.log('✅ Unit 38: 데이터 로드 성공! 단어 수:', data.length);
      console.log('📝 Unit 38: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
    ...
    .catch(err => {
      console.error('❌ Unit 38: 데이터 로드 실패:', err);
      alert('Unit 38 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
}