...
let selectedUnit = 'unit16'; // 기본값: Unit 16
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function fireBullet(playSound = true) {
    ...
    // 로컬 환경과 온라인 환경을 구분하여 경로 설정
    const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
    const assetsPath = isLocal ? './assets/' : '/game/unit16/assets/';
...
function getLatestUnitJsonFile() {
  // data 폴더 내 unit16.json 파일을 사용
  return fetch('/game/unit16/data/unit16.json')
...
function loadWords() {
  // 로컬 환경과 온라인 환경을 구분하여 경로 설정
  const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
  const file = isLocal ? './data/unit16.json' : '/game/unit16/data/unit16.json';
  console.log('🔍 Unit 16: 단어 데이터 로딩 시작:', file);
  
  fetch(file)
    .then(response => {
      console.log('📡 Unit 16: 서버 응답:', response.status, response.ok);
      if (!response.ok) {
    ...
    .then(data => {
      console.log('✅ Unit 16: 데이터 로드 성공! 단어 수:', data.length);
      console.log('📝 Unit 16: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
    ...
    .catch(err => {
      console.error('❌ Unit 16: 데이터 로드 실패:', err);
      alert('Unit 16 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
} 