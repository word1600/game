...
let selectedUnit = 'unit3'; // Unit 3 전용

// Unit 3 전용 게임 - 유닛 선택 기능 제거됨
...
function loadWords() {
  ...
  fetch(file)
    .then(response => {
      ...
      if (!response.ok) {
        throw new Error(`단어 데이터를 불러오지 못했습니다. 상태: ${response.status}`);
      }
      return response.json();
    })
    .then(data => {
      console.log('Unit 3: 단어 데이터 로드 성공, 단어 수:', data.length);
      console.log('Unit 3: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
      ...
      if (ufoInterval) clearInterval(ufoInterval);
      ufoInterval = setInterval(spawnUFO, 1800);
      console.log('Unit 3: 게임 초기화 완료');
    })
    .catch(err => {
      console.error('Unit 3: 단어 데이터 로딩 실패:', err);
      alert('Unit 3: 단어 데이터를 불러오지 못했습니다. 콘솔을 확인해주세요.');
    });
} 