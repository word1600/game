...
let tabletFirstWordHandled = false;
...
let selectedUnit = 'unit9'; // 기본값: Unit 9
...
function pickNewProblemWord() {
  ...
  if (!isPaused) {
    window.ttsTimeout1 = setTimeout(() => {
      if (!isPaused) {
        if (isTabletDevice()) {
          if (!tabletFirstWordHandled) { tabletFirstWordHandled = true; return; }
          speakWordTTSGB(currentWord.en);
        } else {
          speakWordTTSUS(currentWord.en);
          window.ttsTimeout2 = setTimeout(() => { if (!isPaused) speakWordTTSGB(currentWord.en); }, 2000);
        }
      }
    }, 2000);
  }
...
function spawnUFO(forceAnswerUFO = false) {
  ...
  const assetsPath = isLocal ? './assets/' : '/game/unit9/assets/';
...
function getLatestUnitJsonFile() {
  // data 폴더 내 unit8.json 파일을 사용
  return fetch('/game/unit9/data/unit9.json')
...
function loadWords() {
  // 로컬 환경과 온라인 환경을 구분하여 경로 설정
  const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
  const file = isLocal ? './data/unit9.json' : '/game/unit9/data/unit9.json';
  console.log('🔍 Unit 9: 단어 데이터 로딩 시작:', file);
  
  fetch(file)
    .then(response => {
      console.log('📡 Unit 9: 서버 응답:', response.status, response.ok);
      if (!response.ok) {
    ...
    .then(data => {
      console.log('✅ Unit 9: 데이터 로드 성공! 단어 수:', data.length);
      console.log('📝 Unit 9: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
    ...
    .catch(err => {
      console.error('❌ Unit 9: 데이터 로드 실패:', err);
      alert('Unit 9 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
} 