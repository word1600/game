...
function loadWords() {
  ...
  fetch(file)
    .then(response => {
      console.log('📡 Unit 20: 서버 응답:', response.status, response.ok);
      if (!response.ok) {
    ...
    .then(data => {
      console.log('✅ Unit 20: 데이터 로드 성공! 단어 수:', data.length);
      console.log('📝 Unit 20: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
    ...
    .catch(err => {
      console.error('❌ Unit 20: 데이터 로드 실패:', err);
      alert('Unit 20 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
}