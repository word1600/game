...
function getLatestUnitJsonFile() {
  // data 폴더 내 unit10.json 파일을 사용
  return fetch('/game/unit10/data/unit10.json')
...
function loadWords() {
  ...
  fetch(file)
    .then(response => {
      console.log('📡 Unit 10: 서버 응답:', response.status, response.ok);
      if (!response.ok) {
    ...
    .then(data => {
      console.log('✅ Unit 10: 데이터 로드 성공! 단어 수:', data.length);
      console.log('📝 Unit 10: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
    ...
    .catch(err => {
      console.error('❌ Unit 10: 데이터 로드 실패:', err);
      alert('Unit 10 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
} 