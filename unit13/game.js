...
let selectedUnit = 'unit13'; // 기본값: Unit 13
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function fireBullet(playSound = true) {
    ...
    // 로컬 환경과 온라인 환경을 구분하여 경로 설정
    const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
    const assetsPath = isLocal ? './assets/' : '/game/unit13/assets/';
...
function getLatestUnitJsonFile() {
  // data 폴더 내 unit13.json 파일을 사용
  return fetch('/game/unit13/data/unit13.json')
...
function loadWords() {
  // 로컬 환경과 온라인 환경을 구분하여 경로 설정
  const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
  const file = isLocal ? './data/unit13.json' : '/game/unit13/data/unit13.json';
  console.log('🔍 Unit 13: 단어 데이터 로딩 시작:', file);
  
  fetch(file)
    .then(response => {
      console.log('📡 Unit 13: 서버 응답:', response.status, response.ok);
      if (!response.ok) {
    ...
    .then(data => {
      console.log('✅ Unit 13: 데이터 로드 성공! 단어 수:', data.length);
      console.log('📝 Unit 13: 첫 번째 단어:', data[0]);
      ufoWordsData = data;
      refillWordPool();
      pickNewProblemWord();
      updateScore();
    ...
    .catch(err => {
      console.error('❌ Unit 13: 데이터 로드 실패:', err);
      alert('Unit 13 단어 데이터를 불러오지 못했습니다: ' + err.message);
    });
} 